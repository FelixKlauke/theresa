...
public final class LifeCycle {
  ...
  private void buildCallbackActions() {
    ...
    var declaredMethods = handleClass.getDeclaredMethods();
    Arrays.stream(declaredMethods).forEach(this::buildCallbackAction);
  }
  ...
  public void updateState(LifeCycleState lifeCycleState) {
    ...
    var callbacks = lifeCycleCallbacks.getOrDefault(
        lifeCycleState,
        Lists.newArrayList()
    );
  ...
  private void buildCallbackAction(Method declaredMethod) {
    var callback = new MethodLifeCycleStateCallback(declaredMethod);
    if (declaredMethod.isAnnotationPresent(PostConstruct.class)) {
      addCallbackMethod(LifeCycleState.CONSTRUCTION, callback);
    } else if (declaredMethod.isAnnotationPresent(PreDestroy.class)) {
      addCallbackMethod(LifeCycleState.DESTRUCTION, callback);
    } else if (declaredMethod.isAnnotationPresent(WarmUp.class)) {
      addCallbackMethod(LifeCycleState.WARM_UP, callback);
    }
  }
  ...
  private void addCallbackMethod(LifeCycleState lifeCycleState, LifeCycleStateCallback callback) {
    var callbacks = lifeCycleCallbacks.computeIfAbsent(
        lifeCycleState,
