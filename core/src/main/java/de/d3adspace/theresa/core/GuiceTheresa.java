...
public class GuiceTheresa extends AbstractTheresa {
  ...
  GuiceTheresa(LifeCycleRegistry lifeCycleRegistry,
    LifeCycleFactory lifeCycleFactory, Module... externalParentModules) {
    this.lifeCycleRegistry = lifeCycleRegistry;

    Injector externalParentInjector = Guice
        .createInjector(new LifecycleModule(lifeCycleFactory, lifeCycleRegistry));
  ...
  public void startLifeCycle() {
    ...
    boolean validPhase = phaseReference
      .compareAndSet(LifeCyclePhase.LATENT, LifeCyclePhase.STARTING);
    if (!validPhase) {
      throw new IllegalStateException("Can't start life cycle from phase " + phaseReference.get());
    }

    Collection<LifeCycle> lifeCycles = lifeCycleRegistry.findAllLifeCycles();
  ...
  public void stopLifeCycle() {
    ...
    lifeCycles.forEach(lifeCycle -> {
      lifeCycle.updateState(LifeCycleState.DESTRUCTION);
    });
